init()
{
	...
	level.mapvotetext["MapVote"]		= &"^5Apreta ^1[{+attack}]^5 Para Votar";
	level.mapvotetext["TimeLeft"] 		= &"";
	level.mapvotetext["MapVoteHeader"] 	= &"^1Promod ^7Live |^5#^7|^3VZ^4L^1A|^5#^7|";
...
notifySBoard()
{
	iprintln("^2Press^3 [TAB]^2-Para ver  Marcador!");
	wait 6;
	iprintln("^2Press^3 [TAB]^2-Para ver  Marcador!");
	wait 6;
	iprintln("^2Press^3 [TAB]^2-Para ver  Marcador!");
...
SetMapWinner(winner)
{
	...
	iprintlnbold("^6 El  Mapa Ganador Es");
	iprintlnbold("^5" + mapname);
	iprintlnbold("^6" + GetGametypeName(gametype));
	...
	for(i=0;i<temparr.size;)
	{
		switch(temparr[i])
		{
			...
			case "map":
				...
				break;
			default:
				iprintlnbold( "Error En Rotation de Map" );
...
{
	players = getEntArray( "player", "classname" );
	players playlocalsound("promod_end");
